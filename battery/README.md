...
2. Adds `oneUpPower` to `/etc/modules` (loads automatically on boot)
...
Unloads the driver and removes all installed files: the `.ko`, the `/etc/modules` entry, and `/etc/modprobe.d/oneUpPower.conf`.
